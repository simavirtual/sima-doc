...
El proceso de la facturación es el objetivo principal del sistema de control de ingresos y tiene que ver con las actividades operativas, administrativas y
técnicas que se deben realizar para garantizar que los recibos de cada mes se emitan correctamente.

En el proceso de la facturación existen varios factores o actividades que se deben entender y dominar, estos son:
...
En estas actividades se deben aplicar simultáneamente los controles internos y externos
...
como los ha cancelado, su interpretación da como resultado el calculo de su deuda en cualquier momento dado.

Para su correcta interpretación se deben dominar cada uno de los items que lo conforman y las fórmulas que lo gobiernan.

El Estado de cuenta lo conforman los siguientes items:

*MES ACTUAL*::
  Es el mes escogido para un proceso específico.
...
*ESTADO*::
  Es la forma como se pagó el recibo

*FECHA DE PAGO OPORTUNO*::
  Es la primera fecha límite para pagar el recibo oportunamente.

Antes de esta fecha no se cobran intereses y después se empiezan a cobrar intereses por pago extemporáneo.

*FECHA DE PAGO EXTEMPORANEO*::
  Es la segunda fecha límite para poder pagar el recibo.

Después de esta fecha el padre se considera deudor para el mes.

*INTERES POR PAGO EXTEMPORANEO*::
  Es el porcentaje que se cobra por pago extemporáneo o el no pago del recibo. Ejemplo: 2.25%
...
* Para una pensión por valor de $ 100,000 y un interes del 4% para el pago extemporáneo o no pago del recibo.
...
Para el caso de la forma de pago el sistema permite modificar los últimos dos recibos y cuando se aplica un abono el sistema
lo refleja también en los dos últimos recibos.

=== LOS ELEMENTOS DE LA FACTURACION

Los elementos principales de la facturación son aquellos que se reflejan en el  estado de cuenta y los principales son:
...
Los abonos son una disminución a la deuda y de acuerdo a la fecha del abono se toma el saldo anterior o el saldo actual.

Para entender como se aplica un abono se deben dominar los conceptos  del estado de cuenta de un estudiante.

Cuando se aplica un abono el sistema recalcula los registros de los dos últimos recibos para ajustar el abono.
...
*  *Calcular el porcentaje de la proporción de la deuda anterior y los intereses por cobrar*
...
* *Por diferencia calcular el nuevo interés por Cobrar Anterior*
...
* *Calcular el porcentaje de la proporción del la deuda Actual y los intereses por cobrar*
...
* *Por diferencia calcular el nuevo interés por Cobrar Actual*
...
* *Calcular el Interés por Cobrar del Mes*
